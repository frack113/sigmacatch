...
fn handle_info_yml(
    ...
    let rule_id = match triplet::read_rule_id(path) {
        Ok(id) => id,
        Err(e) => {
    ...
    if !triplet::validate_rule_id(&rule_id) {
        warn!(
    ...
    match triplet::validate_triplet(parent, &rule_id) {
        triplet::TripletStatus::Complete => {
        ...
        triplet::TripletStatus::Incomplete(reason) => {
...
mod tests {
    ...
    fn test_parse_info_yml_bom() {
        ...
        let result = triplet::read_rule_id(&info_path);
    ...
    fn test_parse_info_yml_corrupted() {
        ...
        assert!(triplet::read_rule_id(&info_path).is_err());
    }
