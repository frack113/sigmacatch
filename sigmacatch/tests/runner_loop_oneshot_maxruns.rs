...
//! One-shot interval cycle (`runner::run_with_cli`, AD-6): a collector that
//! keeps its channel open (until the runner signals shutdown) so the one-shot
//! 30-second generation interval actually fires. The cycle generates + uploads
//! (offline: commit skipped) and the `--max-runs 1` limit stops the loop.
//!
...
/// Sends its event, then holds the channel open until the runner asks for
/// shutdown — without this the one-shot loop would exit on the closed channel
/// before the 30-second generation interval could fire.
...
#[tokio::test]
async fn oneshot_max_runs_interval_cycle_generates_uploads_and_exits() {
    ...
    // The one-shot generation interval is 30 s, so the full run takes ~30 s.
    tokio::time::timeout(
    ...
    .await
    .expect("one-shot max-runs run must not hang")
    .expect("one-shot max-runs pipeline must finish");
