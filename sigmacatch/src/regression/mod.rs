...
use crate::types::{Alert, RegressionHeader};
use tracing::{error, info, info_span, warn};
use uuid::Uuid;

use crate::regression::format::DATA_EXTENSIONS;
pub use crate::regression::format::DataFormat;
use crate::regression::info::{InfoYml, TestConfig};
use crate::regression::logtype::LogType;
...
fn data_file_exists(dir: &Path, rule_id: &Uuid, format: DataFormat) -> bool {
    let ext = format.ext();
    let candidate = crate::regression::long_path::long_path(&dir.join(format!("{rule_id}.{ext}")));
...
impl RegressionData {
    ...
    fn generate(&self) -> Result<()> {
        ...
        let info_path = crate::regression::long_path::long_path(&rule_dir.join("info.yml"));
        info.save(&info_path)?;
...
mod tests {
    use super::*;
    use crate::regression::format::MAX_DATA_BLOB_SIZE;
  ...
  <EventData>
    ...
    fn test_noid_alert_generates_valid_output() {
        ...
        let events = crate::evtx_reader::parse_evtx_file(&evtx_path).unwrap();
