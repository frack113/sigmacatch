...
#[allow(dead_code)]
pub fn build_logsource_to_channels(custom_map: &HashMap<String, String>) -> HashMap<String, Vec<ChannelTarget>> {
...
mod tests {
    ...
    fn test_resolve_channel_known() {
        let ls = resolve_logsource("Microsoft-Windows-Sysmon/Operational", "", 1, &HashMap::new());
    ...
    fn test_build_logsource_to_channels_with_custom() {
        let mut custom = HashMap::new();
        custom.insert("Custom-Channel/Operational".to_string(), "custom_service".to_string());
        ...
        assert!(targets.iter().any(|t| t.channel == "Custom-Channel/Operational"));
    }
