...
writer (`evtx_writer` module of `sigmacatch::regression`, deterministic, no retry). The exported file is
...
flat (`{message, program, host?}`, `host` optional) for syslog and structured
(`{stamp, type, node?, fields}`) for auditd on Linux. The pipeline writes a single
