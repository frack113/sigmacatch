...
- **Valid binary EVTX**: `<rule_id>.evtx` is written via `EvtWriteFile` API (Windows) or raw XML (non-Windows).
  The raw Winevt XML (`WinevtEvent.raw_xml`) is written directly to the EVTX file.
  On Windows, the format is identical to that generated by Winevt (valid for hayabusa/chainsaw).
