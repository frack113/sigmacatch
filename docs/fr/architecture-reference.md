...
2. Acquérir les règles SigmaHQ (grit-lib clone/fetch) + détecter le fork + créer la branche
...
| `reqwest` (blocking + async) | client HTTP pour le transport git + détection de fork (API GitHub) |
